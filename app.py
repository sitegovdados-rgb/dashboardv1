...
# Custom CSS for Premium Look (Glassmorphism, colors, margins)
st.markdown("""
<style>
    /* Global styles */
    .stApp {
...
COORDS = {
    'Manguinhos': (-22.8808, -43.2515),
    'Tijuquinha': (-23.0031, -43.3130),
    'CIEP Professor Lauro De Oliveira Lima': (-22.9910, -43.3350),
    'Campo do Abóbora - Jacarézinho': (-22.8894, -43.2562),
    'Ciep Presidente João Goulart': (-22.9785, -43.1942),
    'Itanhangá': (-22.9922, -43.3155),
    'Ipanema': (-22.9836, -43.2018),
    'Muzema': (-22.9961, -43.3276),
    'Cantagalo-Pavão-Pavãozinho': (-22.9790, -43.1950),
    'Muzema CIEP Professor Lauro De Oliveira Lima Itanhangá': (-22.9930, -43.3300),
    'Campo do Abóbora - JacarézinhoManguinhos': (-22.8850, -43.2540),
    'Itanhangá Tijuquinha': (-22.9980, -43.3140),
    'Rio das Pedras': (-22.9902, -43.3364)
}
...
@st.cache_data
def load_social_data(file_path="dashboard-social-geral.csv"):
    return load_default_file(file_path)

@st.cache_data
def load_urbanism_data(file_path="dashboard-urbanismo-geral.csv"):
    return load_default_file(file_path)
...
def render_metric_card(title, value, delta=None, color="#00D4B2"):
    delta_html = f"<div style='color: #94A3B8; font-size: 0.85rem; margin-top: 6px; font-weight: 500;'>{delta}</div>" if delta else ""
    card_html = f"""
    <div class="metric-card">
        <div style="color: #94A3B8; font-size: 0.85rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;">{title}</div>
...
with tab_geral:
    ...
    with col_left:
        ...
        fig_comp.update_layout(
            title="Relação: Iniciativas Sociais vs. Investimento em Obras",
            yaxis=dict(
...
with tab_social:
    ...
    with col_s1:
        st.metric("Total de Projetos", len(df_social_filtered))
    with col_s2:
        media_atendidos_mensal = df_social_filtered['Média Atendidos (Mensal)'].sum()
        st.metric("Média de Atendimentos/Mês", f"{media_atendidos_mensal:,.0f}")
    with col_s3:
        # Calcular Taxa de Cobertura de Dados (Média das colunas binárias de dados que não estão como 'Aguardando Informação')
        cols_coleta_social = [
        ...
        st.metric("Maturidade de Coleta de Dados", f"{pct_cobertura:.1f}%")
        
    with col_s4:
        concluidos_soc = len(df_social_filtered[df_social_filtered['Status'] == 'Concluída'])
        st.metric("Projetos Concluídos", concluidos_soc, f"{(concluidos_soc / len(df_social_filtered) * 100):.1f}% do total" if len(df_social_filtered) > 0 else "0%")
...
with tab_urbanism:
    ...
    with col_u1:
        st.metric("Obras Cadastradas", len(df_urb_filtered))
    with col_u2:
        st.metric("Orçamento Planejado", f"R$ {tot_previsto:,.2f}")
    with col_u3:
        st.metric("Orçamento Executado (Pago)", f"R$ {tot_realizado:,.2f}", f"{(tot_realizado/tot_previsto*100):.1f}% do previsto" if tot_previsto > 0 else "0%")
    with col_u4:
        obras_em_exec = len(df_urb_filtered[df_urb_filtered['Status'] == 'Em execução'])
        st.metric("Obras Ativas (Em Execução)", obras_em_exec)
    ...
    with col_graph_u1:
        ...
        fig_sub.update_layout(
            ...
            plot_bgcolor='rgba(0,0,0,0)',
            font=dict(color="#94A3B8"),
            yaxis=dict(gridcolor="rgba(255, 255, 255, 0.05)", title="Valores em Reais (R$)"),
            legend=dict(x=0.01, y=0.99, bgcolor="rgba(15,23,42,0.8)", bordercolor="rgba(255,255,255,0.05)")
        )
